...
export default class I2PInbound implements InboundInterface {
  ...
  async start(address: AddressInfo) {
    try {
      ...
      const privateKey = secrets.get("i2pPrivateKey");
      const publicKey = secrets.get("i2pPublicKey");
      ...
      secrets.set("i2pPrivateKey", this.forward.getPrivateKey());
      secrets.set("i2pPublicKey", this.forward.getPublicKey());
    } catch (error) {
      this.running = false;

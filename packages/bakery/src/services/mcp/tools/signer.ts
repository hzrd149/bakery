...
mcpServer.tool(
  "set_user_pubkey",
  "Change the user to a new pubkey. only use this to login in read-only mode, if the user wants to sign events use the signer tools",
...
mcpServer.tool("get_user_pubkey", "Gets the users pubkey", {}, async () => {
  const pubkey = bakeryConfig.data.owner;
  if (!pubkey) return { content: [{ type: "text", text: "No user pubkey set" }] };
...
mcpServer.tool("get_setup_status", "Checks if the bakery needs to be setup", {}, async () => {
  if (!bakeryConfig.data.owner)
    return { content: [{ type: "text", text: "Missing user pubkey, please setup the user to finish bakery setup" }] };
...
mcpServer.tool(
  ...
  "Creates a nostrconnect:// URI for the user to setup their signer",
